...
(function() {
  var APIKey, APIUrl, DetailAPIUrl, PhotoAPIUrl, config, deg2rad, findEfoos, getDetailInfo, getDistanceFromLatLonInKm, getEfoosByReq, getPhotoByReference, misc, normalizeDistance, priceLevels, radiusKeyboard;
  ...
  getEfoosByReq = function(datas, reqData, cb) {
    var data, distance, index, openNow, priceLevel, rating;
    index = reqData.current - 1;
    data = datas[index];
    if (data.opening_hours != null) {
      openNow = data.opening_hours.open_now ? 'Да' : 'Нет';
    } else {
    ...
    return getDetailInfo(data.place_id).then((function(_this) {
      return function(response) {
        var address, keyboard, phoneNumber, raw, result;

...
var CB_DELAY, config, formatDate, keyboard, logger, misc, msgCache, pq, quotes, sanitizeHtml,
...
module.exports = {
  ...
  onMsg: function(msg) {
    var cmd, hdr, inlineMode, num, ownerId, quote, reply, txt;
    ...
    if (quote != null) {
      if (inlineMode) {
        return this.sendInline(msg, quote);
      } else {
  ...
  getQuoteFull: function(quote) {
    return (this.getQuoteHeader(quote, true)) + "\n\n" + (this.getQuoteText(quote));
  },
  sendInline: function(msg, quote) {
    var context;
    context = {
      quote: quote
    };
    return msg.send(this.getQuoteFull(quote), {
      parseMode: 'HTML',
      preview: false,
  ...
  onCallback: function(context, cb, msg) {
    ...
    switch (cb.data) {
      ...
      case 'prev':
        ...
        quote = null;
        while ((quote == null) && num > 1) {
          num -= 1;
          quote = quotes.getByNumber(num);
        }
        if (quote != null) {
          this.updateInline(context, quote);
        }
        break;
      case 'next':
        num = context.quote.num;
  ...
  vote: function(context, cb, isUp) {
    ...
    if (quotes.vote(num, null, cb.from.id, isUp) != null) {
      ...
      this.updateInline(context, context.quote, true);
    }
  },
  updateInline: function(context, quote, force) {
    if (force == null) {
      force = false;
    }
    if (force || context.quote.num !== quote.num) {
      context.msg.edit(this.getQuoteFull(quote), {
        parseMode: 'HTML',
        preview: false,
        inlineKeyboard: keyboard
      }).then((function(_this) {
